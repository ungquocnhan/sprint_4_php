    ...
    <form action="{{route('products.update')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <input type="text"
                   ...
                   value="{{old('name') ?? $productEdit->name}}">
            @error('name')
        ...
        <div class="mb-3">
            ...
            <input type="number"
                   ...
                   value="{{old('quantity_exists') ?? $productEdit->quantity_exists}}">
        ...
        <div class="mb-3">
            ...
            <input type="text"
                   ...
                   value="{{old('size') ?? $productEdit->size}}">
            @error('size')
            <span style="color: red">{{$message}}</span>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <select name="flag_promoted" id="flag_promoted" class="form-control">
                <option value="0" {{old('flag_promoted') == 0 ||
                $productEdit->flag_promoted == 0 ?
                'selected' : false}}>Không khuyến mãi
                </option>
                <option value="1" {{old('flag_promoted') == 1 ||
                $productEdit->flag_promoted == 1 ?
                'selected' : false}}>Khuyến mãi
                </option>
            </select>
        </div>
                ...
                @if(!empty(getAllCoverageDensity()))
                    @foreach(getAllCoverageDensity() as $item)
                        <option value="{{$item->id}}"
                            {{old('coverage_density_id') == $item->id ||
                            $productEdit->coverage_density_id == $item->id ?
            ...
            <select name="frequency_band_id" id="frequency_band_id" class="form-control">
                ...
                @if(!empty(getAllFrequencyBand()))
                    @foreach(getAllFrequencyBand() as $item)
                        <option value="{{$item->id}}"
                            {{old('frequency_band_id') == $item->id ||
                            $productEdit->frequency_band_id == $item->id ?
                            'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="guarantee_id" id="guarantee_id" class="form-control">
                ...
                @if(!empty(getAllGuarantee()))
                    @foreach(getAllGuarantee() as $item)
                        <option value="{{$item->id}}"
                            {{old('guarantee_id') == $item->id ||
                            $productEdit->guarantee_id == $item->id ?
                            'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="made_in_id" id="made_in_id" class="form-control">
                ...
                @if(!empty(getAllMadeIn()))
                    @foreach(getAllMadeIn() as $item)
                        <option value="{{$item->id}}"
                            {{old('made_in_id') == $item->id ||
                            $productEdit->made_in_id == $item->id ?
                             'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="manufacture_id" id="manufacture_id" class="form-control">
                ...
                @if(!empty(getAllManufacture()))
                    @foreach(getAllManufacture() as $item)
                        <option value="{{$item->id}}"
                            {{old('manufacture_id') == $item->id ||
                            $productEdit->manufacture_id == $item->id ?
                             'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="promotion_id" id="promotion_id" class="form-control">
                ...
                @if(!empty(getAllPromotion()))
                    @foreach(getAllPromotion() as $item)
                        <option value="{{$item->id}}"
                            {{old('promotion_id') == $item->id ||
                            $productEdit->promotion_id == $item->id ?
                             'selected' : false}}>
            ...
            <select name="speed_wifi_id" id="speed_wifi_id" class="form-control">
                ...
                @if(!empty(getAllSpeedWifi()))
                    @foreach(getAllSpeedWifi() as $item)
                        <option value="{{$item->id}}"
                            {{old('speed_wifi_id') == $item->id ||
                            $productEdit->speed_wifi_id == $item->id ?
                             'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="standard_network_id" id="standard_network_id" class="form-control">
                ...
                @if(!empty(getAllStandardNetwork()))
                    @foreach(getAllStandardNetwork() as $item)
                        <option value="{{$item->id}}"
                            {{old('standard_network_id') == $item->id ||
                            $productEdit->standard_network_id == $item->id ?
                             'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="type_anteing_id" id="type_anteing_id" class="form-control">
                ...
                @if(!empty(getAllTypeAnteing()))
                    @foreach(getAllTypeAnteing() as $item)
                        <option value="{{$item->id}}"
                            {{old('type_anteing_id') == $item->id ||
                            $productEdit->type_anteing_id == $item->id ?
                             'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="type_device_id" id="type_device_id" class="form-control">
                ...
                @if(!empty(getAllTypeDevice()))
                    @foreach(getAllTypeDevice() as $item)
                        <option value="{{$item->id}}"
                            {{old('type_device_id') == $item->id ||
                            $productEdit->type_device_id == $item->id ?
                             'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="user_connect_id" id="user_connect_id" class="form-control">
                ...
                @if(!empty(getAllUserConnect()))
                    @foreach(getAllUserConnect() as $item)
                        <option value="{{$item->id}}"
                            {{old('user_connect_id') == $item->id ||
                            $productEdit->user_connect_id == $item->id ?
                             'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="button_support_id" id="button_support_id" class="form-control">
                ...
                @if(!empty(getAllButtonSupport()))
                    @foreach(getAllButtonSupport() as $item)
                        <option value="{{$item->id}}"
                            {{old('button_support_id') == $item->id ||
                            $productEdit->button_support_id == $item->id ?
                             'selected' : false}}>{{$item->name}}</option>
                    @endforeach
                @endif
            </select>
            ...
            <select name="anteing_id" id="anteing_id" class="form-control">
                ...
                @if(!empty(getAllAnteing()))
                    @foreach(getAllAnteing() as $item)
                        <option value="{{$item->id}}"
                            {{old('anteing_id') == $item->id ||
                            $productEdit->anteing_id == $item->id ?
                             'selected' : false}}>{{$item->quantity}}</option>
                    @endforeach
                @endif
            </select>
            @error('anteing_id')
                ...
                <div class="row" style="margin: 10px 10px">
                    @if(!empty($images))
                        @foreach($images as $item)
                            <div class="col-4">
                                <input type="number" hidden name="idImage[]" value="{{$item['idImage']}}">
                                <img src="{{$item['image']}}" style="height: 160px; width: 100%" alt="">
                            </div>
                        @endforeach
                    @endif
                </div>
    ...
    <script>
        $(document).ready(function () {
            $('#images').fileinput({
                ...
                initialPreview: [
                    @foreach($images as $item)
                        "{{asset('storage/app/image/' . $item['name']) }}"
                    @endforeach
                ],
                initialPreviewConfig: [
                    ...
                    {
                        caption: "{{ $item['name'] }}",
                        size: {{ Storage::exists('storage/app/image/' . $item['name']) ?
                                Storage::size('storage/app/image/' . $item['name']) : 0 }},
                        width: "120px",
                        url: "{{$item['image']}}",
                        key: {{ ($index + 1) }}
                    },
